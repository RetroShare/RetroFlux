...
my $template = HTML::Template->new(filename => 'template/download.tmpl');
retrofunc::loadDefaultTemplateParams($template);
$template->param(SHARESTOPBLUEICON => retroconfig::getShareStopBluelink());
$template->param(SHAREPAUSEBLUEICON => retroconfig::getSharePauseBluelink());
$template->param(SHARERESTARTBLUEICON => retroconfig::getShareRestartBluelink());
$template->param(SORTICONLINK => retroconfig::getSortIconlink());

retrofunc::ssh2Connect();
my @dwnlist = retrofunc::getDownloadList();
...
if($rows) {
   ...
   for my $ergeb (keys %dwnhash) {
          my %row_data;
          my $rawname = $dwnhash{$ergeb}{'name'};
          my $fname = $rawname;
          my $aname = uri_escape($rawname);
